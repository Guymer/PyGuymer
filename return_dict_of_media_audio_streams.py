...
#                                                                                            #
# https://github.com/Guymer/PyGuymer3/blob/master/return_dict_of_media_audio_streams.py      #
##############################################################################################
        ...
        if u"PlayList" in nfo:
            ...
            if u"PlayItems" in nfo[u"PlayList"]:
                ...
                for PlayItem in nfo[u"PlayList"][u"PlayItems"]:
                    ...
                    for name in [u"PrimaryAudioStreamEntries", u"SecondaryAudioStreamEntries"]:
                        # Loop over AudioStreamEntries ...
                        for AudioStreamEntry in PlayItem[u"STNTable"][name]:
                            # Check keys ...
                            if u"StreamEntry" in AudioStreamEntry and u"StreamAttributes" in AudioStreamEntry:
                                # Check keys ...
                                if u"RefToStreamPID" in AudioStreamEntry[u"StreamEntry"] and u"LanguageCode" in AudioStreamEntry[u"StreamAttributes"]:
                                    # Loop over streams ...
                                    for stream in ans.iterkeys():
                                        # Check if this is the stream ...
                                        if AudioStreamEntry[u"StreamEntry"][u"RefToStreamPID"] == ans[stream][u"id"]:
                                            # Add language code to the stream
                                            # information ...
                                            ans[stream][u"langcode"] = AudioStreamEntry[u"StreamAttributes"][u"LanguageCode"]
